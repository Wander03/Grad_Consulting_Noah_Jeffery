...
read_participant_data <- function(part_dir) {
  participant <- gsub("Participant|_15min", "", basename(part_dir))
...
full_data <- map_dfr(participant_dirs, read_participant_data) %>%
  mutate(
    participant = factor(participant, levels = as.character(sort(as.numeric(unique(participant))))),
    device_location = case_when(
      device_name == 'fe:77:90:13:0e:6b' ~ 'neck',
      device_name == 'ea:e4:fd:d7:dc:2a' ~ 'mid_back',
      device_name == 'cd:75:20:e1:ef:ef' ~ 'lower_back',
      TRUE ~ NA
      )
    ) %>%
  select(participant, device_location, time, chip_time, starts_with('angle')) %>%
  arrange(participant)

write.csv(full_data, file = 'free_15min.csv', row.names = F)
```
...
summary_df %>%
  ggplot() +
  geom_tile(aes(x = device_name, y = participant_number, fill = median)) +
  scale_fill_distiller(palette = 'Spectral') +
...
## Plots of Angle x over Time/Chip Time
...
```{r}
full_data %>%
  filter(participant_number %in% seq(1, 5)) %>% # Look at first 5 participants
  ggplot() +
  ...
  theme_bw() +
  labs(title = "Angle X by Chip Time by Device",
       subtitle = "Participants 1-5")
```
...
full_data %>%
  filter(participant_number == 5) %>%
    ggplot() +
    geom_line(aes(x = normalized_chip_time, y = angle_x, color = device_name)) +
...
full_data %>%
  ...
  labs(title = "Normalized Angle X by Chip Time") +
  theme_bw() +
  ylim(-50, 50)
...
full_data %>%
  ...
  labs(title = "Normalized Angle X by Time") +
  theme_bw() +
  ylim(-50, 50)
...
full_data %>%
  filter(device_name == 'fe') %>%
  ggplot(aes(x = normalized_angle_x, y = participant_number, fill = ..x..)) +
    geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) +
    scale_fill_viridis(name = "angle_x", option = "C") +
    labs(title = 'Distribution of Time Spent in Normalized Angle X by Participant', subtitle = 'fe') +
    theme_ipsum() +
      theme(
        legend.position="none",
        panel.spacing = unit(0.1, "lines"),
...
full_data %>%
  filter(device_name == 'ea') %>%
  ggplot(aes(x = normalized_angle_x, y = participant_number, fill = ..x..)) +
    geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) +
    scale_fill_viridis(name = "angle_x", option = "C") +
    labs(title = 'Distribution of Time Spent in Normalized Angle X by Participant', subtitle = 'ea') +
    theme_ipsum() +
      theme(
        legend.position="none",
        panel.spacing = unit(0.1, "lines"),
...
full_data %>%
  filter(device_name == 'cd') %>%
  ggplot(aes(x = normalized_angle_x, y = participant_number, fill = ..x..)) +
    geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) +
    scale_fill_viridis(name = "angle_x", option = "C") +
    labs(title = 'Distribution of Time Spent in Normalized Angle X by Participant', subtitle = 'cd') +
    theme_ipsum() +
      theme(
        legend.position="none",
        panel.spacing = unit(0.1, "lines"),
