...
full_data %>%
  # filter(participant_number %in% seq(1, 5)) %>%
  ggplot() +
    geom_line(aes(x = chip_time, y = angle_x, color = participant_number)) +
...
full_data %>%
  # filter(participant_number %in% seq(1, 5)) %>%
  ggplot() +
    geom_line(aes(x = time, y = angle_x, color = participant_number)) +
    facet_wrap(vars(device_name), nrow = 3, ncol = 1) +
    theme(
    strip.text = element_text(size = 8),  # Adjust label size
    axis.text = element_text(size = 6),   # Adjust axis text size
...
full_data %>%
  # filter(participant_number %in% seq(1, 5)) %>%
  ggplot() +
    geom_line(aes(x = normalized_chip_time, y = angle_x, color = participant_number)) +
    facet_wrap(vars(device_name), nrow = 3, ncol = 1) +
    theme(
    strip.text = element_text(size = 8),  # Adjust label size
    axis.text = element_text(size = 6),   # Adjust axis text size
...
full_data %>%
  ggplot(aes(x = angle_x, y = participant_number, fill = device_name)) +
...
full_data %>%
  filter(device_name == 'fe:77:90:13:0e:6b') %>%
  ggplot(aes(x = angle_x, y = participant_number, fill = ..x..)) +
    geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) +
...
full_data %>%
  filter(device_name == 'ea:e4:fd:d7:dc:2a') %>%
  ggplot(aes(x = angle_x, y = participant_number, fill = ..x..)) +
    geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) +
    scale_fill_viridis(name = "angle_x", option = "C") +
    labs(title = 'Distribution of Time Spent in Angle X by Participant') +
...
full_data %>%
  filter(device_name == 'cd:75:20:e1:ef:ef') %>%
  ggplot(aes(x = angle_x, y = participant_number, fill = ..x..)) +
    geom_density_ridges_gradient(scale = 3, rel_min_height = 0.01) +
    scale_fill_viridis(name = "angle_x", option = "C") +
    labs(title = 'Distribution of Time Spent in Angle X by Participant') +
