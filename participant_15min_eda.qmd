...
full_data <- map_dfr(participant_dirs, read_participant_data) %>%
  mutate(
    ...
    device_name = case_when(
      device_name == 'fe:77:90:13:0e:6b' ~ 'fe',
      device_name == 'ea:e4:fd:d7:dc:2a' ~ 'ea',
      device_name == 'cd:75:20:e1:ef:ef' ~ 'cd',
      TRUE ~ NA
      )
    ) %>%
...
write.csv(full_data, file = 'combined_15min.csv', row.names = F)
```
