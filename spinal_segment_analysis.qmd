...
plot_classifications_advanced <- function(df, PI, loc, p) {
  ...
  # plot_data_classified <- plot_data_classified %>%
  #   mutate(
  #     posture_group = factor(posture_group, levels = c(postures, "Unclassified"))
  #   )
  ...
  # Add unclassified areas (below lowest lower and above highest upper)
  unclassified_ranges <- data.frame(
    posture = "Unclassified",
    xmin = 0,
    xmax = 1,
    ymin = c(-Inf, max(PI$upper.PL)),
    ymax = c(min(PI$lower.PL), Inf)
  )
  ...
  # Generate color palette (one color per posture + gray for unclassified)
  posture_colors <- setNames(
    c(scales::hue_pal()(length(postures)), "gray80"),
    c(postures, "Unclassified")
  )
